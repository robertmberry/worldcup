...
do
  if [[ $YEAR != year ]]
    ...
    if [[ $RESULT_INSERT_WINNER == 'INSERT 0 1' ]]
      ...
      # INSERT GAME
      RESULT_INSERT_GAME=$($PSQL "INSERT INTO games (year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES ($YEAR, '$ROUND', $WINNER_ID, $OPPONENT_ID, $WINNER_GOALS, $OPPONENT_GOALS)")
      if [[ $RESULT_INSERT_GAME == 'INSERT 0 1' ]]
        then
          echo -e "\nInserted game: $WINNER vs. $OPPONENT into games table\n"
        fi
    fi
done
